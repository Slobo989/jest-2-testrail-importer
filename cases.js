class TestCase {
  async importCases(projectId, selectedSuiteId, selectedSectionId, testCases, api) {
    for (let j = 0; j < testCases.length; j++) {
      ...
      const oldCases = await api.getCases(projectId, { suite_id: selectedSuiteId, section_id: selectedSectionId })
      ...
      if (!exists) {
        await api.addCase(selectedSectionId, { title: newCaseName, ['custom_exectype']: 2 })
      }
    }
  }
  ...
  async createCases(projectId, suiteId, sectionId, testCases, api) {
    for (let l = 0; l < testCases.length; l++) {
      ...
      if (!exists3) {
        subsectionId = (await api.addSection(projectId, {
          suite_id: suiteId,
          parent_id: sectionId,
          name: subsectionName,
        })).id
      ...
      // The new case name is checked against existing cases from the TestRail API (oldCases)
      const oldCases = await api.getCases(projectId, { suite_id: suiteId, section_id: subsectionId })
      ...
      // If a test case with the same name doesn't exist, a new test case is created
      if (!exists) {
        await api.addCase(subsectionId, { title: newCaseName, ['custom_exectype']: 2 })
      }
    }
  }
}
