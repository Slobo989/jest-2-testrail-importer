class Section {
  async selectSection(projectId, selectedSuiteId, api, consoleApp) {
    ...
    do {
      ...
      // If Create new section is selected, it first checks if the section exists - if it does, it asks for the section selection again (the do-while loop and repeat variable)
      if (selectedSection === 'Create new section') {
        ...
        let exists = false
        // For loop checks if the new section exists in the section list (oldSections) - it compares all the names to the name of the new section
        for (let j = 0; j < oldSections.length; j++) {
          if (oldSections[j].name === newSection.name) {
            // If it finds the same section, repeat and exists is set to true, so the do-while loop repeats and a new section is not created
            repeat = true
            exists = true
